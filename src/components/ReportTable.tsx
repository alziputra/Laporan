...
export const ReportTable: React.FC<ReportTableProps> = ({
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, selectedCategory, statusFilter, dateFilter, methodFilter, itemsPerPage]);
  ...
  const stats = useMemo(() => {
    ...
    // Method counts
    const remoteCount = reports.filter(r => (r.metodePenanganan || '').toLowerCase().includes('remote')).length;
    const guideCount = reports.filter(r => (r.metodePenanganan || '').toLowerCase().includes('guide')).length;
    const visitCount = reports.filter(r => (r.metodePenanganan || '').toLowerCase().includes('visit')).length;
  ...
  const filteredReports = useMemo(() => {
    return reports.filter((item) => {
      ...
      const matchesMethod = methodFilter === 'Semua' || item.metodePenanganan === methodFilter;

      return matchesSearch && matchesCategory && matchesStatus && matchesDate && matchesMethod;
    });
  }, [reports, searchTerm, selectedCategory, statusFilter, dateFilter, methodFilter]);
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      {/* 1. Dynamic KPI / Stats Overview Cards (Optimized for Mobile) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
        {/* Card 1: Total Tiket */}
        <div className="bg-white p-3 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between transition-all">
          <div className="min-w-0">
            <p className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wider truncate">Total Laporan</p>
            <h3 className="text-lg sm:text-2xl lg:text-3xl font-black text-slate-800 mt-0.5 sm:mt-1">
          ...
          <div className="w-8 h-8 sm:w-11 sm:h-11 rounded-xl bg-pegadaian-50 text-pegadaian-700 flex items-center justify-center border border-pegadaian-100 shrink-0 ml-2">
        ...
        <div className="bg-white p-3 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between transition-all">
          <div className="min-w-0">
            <p className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wider truncate">Hari Ini</p>
            <h3 className="text-lg sm:text-2xl lg:text-3xl font-black text-emerald-700 mt-0.5 sm:mt-1">
              {stats.todayCount}
            </h3>
          ...
          <div className="w-8 h-8 sm:w-11 sm:h-11 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-100 shrink-0 ml-2">
        ...
        <div className="bg-white p-3 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between transition-all">
          <div className="min-w-0">
            ...
            <h3 className="text-lg sm:text-2xl lg:text-3xl font-black text-teal-700 mt-0.5 sm:mt-1 font-mono">
            ...
            <p className="text-[10px] sm:text-[11px] text-teal-600 font-bold mt-0.5 flex items-center gap-1 truncate">
              ...
              <span>{stats.avgMinutes <= 15 ? 'Respon Cepat ⚡' : 'Standar'}</span>
            </p>
          </div>
          <div className="w-8 h-8 sm:w-11 sm:h-11 rounded-xl bg-teal-50 text-teal-600 flex items-center justify-center border border-teal-100 shrink-0 ml-2">
            <Zap className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
        </div>

        {/* Card 4: Dominasi Metode */}
        <div className="bg-white p-3 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between transition-all">
          <div className="min-w-0 flex-1">
            <p className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wider truncate">Sebaran Metode</p>
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              <span className="text-[10px] sm:text-xs font-extrabold text-sky-700 bg-sky-50 px-1.5 py-0.5 rounded-md border border-sky-200">
                {stats.remoteCount} Remote
              </span>
              <span className="text-[10px] sm:text-xs font-extrabold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded-md border border-emerald-200">
                {stats.guideCount} Guide
              </span>
            </div>
            <p className="text-[10px] text-slate-400 font-medium mt-0.5 truncate">
              {stats.visitCount} On-Site Visit
            </p>
          </div>
          <div className="w-8 h-8 sm:w-11 sm:h-11 rounded-xl bg-sky-50 text-sky-600 flex items-center justify-center border border-sky-100 shrink-0 ml-2">
      ...
      {/* 3. Main Data Card with Toolbar, Mobile Card View & Desktop Table */}
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xs border border-slate-200/80 overflow-hidden">
        ...
        <div className="p-3.5 sm:p-5 border-b border-slate-200/80 bg-gradient-to-r from-slate-50 via-white to-slate-50 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 sm:flex sm:flex-wrap items-center gap-2">
            ...
            <div className="relative flex-1 min-w-[160px]">
              ...
              <input
                type="text"
                placeholder="Cari user, unit, issue..."
                value={searchTerm}
            ...
            {/* Date & Method Filter Row on mobile */}
            <div className="grid grid-cols-2 sm:flex sm:items-center gap-2">
              <input
                type="date"
                value={dateFilter}
                onChange={(e) => setDateFilter(e.target.value)}
                className="w-full sm:w-auto px-2.5 py-2 bg-slate-50 rounded-xl border border-slate-200 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-pegadaian-500 cursor-pointer"
            ...
            {/* Reset & Mobile Action Buttons */}
            <div className="flex items-center gap-2 justify-between sm:justify-start">
              {(searchTerm || dateFilter || selectedCategory !== 'Semua' || methodFilter !== 'Semua') && (
                <button
                  onClick={() => {
                    setSearchTerm('');
                    setDateFilter('');
        ...
        {/* 4. Unified Responsive Table View (Consistent across mobile & desktop) */}
          ...
          <table className="w-full text-left text-xs align-middle border-collapse">
            <thead>
              <tr className="bg-gradient-to-r from-slate-900 via-pegadaian-900 to-slate-900 text-white uppercase text-[11px] font-extrabold tracking-wider border-b-2 border-emerald-500/50">
                ...
                <th className="py-3.5 px-4 min-w-[230px] border-r border-slate-700/60">Deskripsi Permohonan</th>
                <th className="py-3.5 px-3 min-w-[100px] border-r border-slate-700/60 text-center">Metode</th>
                <th className="py-3.5 px-4 min-w-[230px] border-r border-slate-700/60">Solusi Issue</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-3 text-center border-r border-slate-100 whitespace-nowrap">
                        {getMethodBadge(item.metodePenanganan)}
                      </td>
